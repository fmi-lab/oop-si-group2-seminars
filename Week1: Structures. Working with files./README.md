...
Но стойността на вече инициялизиран масив от символи не може да бъде променяна
...
ако с него трябва да се предава и дължината на масива като допълнителен
параметър.

Както се казва: **"The first step to learning C is understanding that pointers and
arrays are the same thing. The seconds step is understanding that pointers and
...
използвате допълнителен аргумент за размер във функциите, които приемат масиви
като аргументи и ги обработват по някакъв начин.
...
Вместо него се използва `name_of_stream.getline(destination,
sizeof(destination));`. Идентична е употребата както за входния потребителски
поток `cin`, така и за файловите потоци.

...
**Извод:** Статичните променливи се пазят в stack-а и паметта, която заемат се
установява по време на компилация (compile time). Също така те заемат
последователни блокове от паметта. 
Динамичните променливи се пазят в heap-а и паметта, която заемат се
установява по време на изпълнение (runtime) и необходимата за деларирането им
памет се заделя от места от където това е възможно. Затова размерът им не може
да бъде установен със `sizeof()`.
